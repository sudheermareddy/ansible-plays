...
TABLE_NAME="labs"
CONTAINER_NAME="tl-reports/lab" # container name changes
...
then
        for status in draft publish review
        do
                fileNameExt=`date +%Y_%m_%d_%H_%M_%S`
                fileName=$TABLE_NAME"_"$status"_"$fileNameExt".json"
                echo "Export for data started, file name : $fileName"
               /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields({'status': {'$status' : { 'createdDTS': true}}}).pluck({'status': {'$status' : { 'labId': true, 'authors': true, 'category': true, 'createdDTS': true, 'labName': true, 'labType': true, 'longDescription': true, 'orgId': true, 'publishType': true, 'publisherName': true, 'shortDescription': true, 'skillLevel': true, 'status': true, 'tags': true, 'tenantId': true, 'userId': true, 'updatedDTS': true} }}).map((data)=> data('status')('$status'))" > $fileName
