...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields('trainingRequestedTimestamp','launchTime').filter(r.row('launchTime').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).pluck('launchState','trainingRequestedTimestamp','user_training_id','orgId','userEmail','userId','userName','modeType',{'finalResponse':{'status': true}},'transfer_group','trainingDeploymentId','launchTime','cleanupStatus','channelData','publisherOrg','isEmailSent','labId','labName').map(function(row){return row.merge({'trainingRequestedTimestamp' : row('trainingRequestedTimestamp').date().toISO8601().slice(0,-6),'launchTime' : row('launchTime').date().toISO8601().slice(0,-6)})})" > $fileName
