...
CONTAINER_NAME="tl-reports/review-ratings"

HOST="rdbokenprtl"
#HOST="rdbprtl"
PORT="28015"
DB_NAME="tl_qloudable_trainings"
TABLE_NAME="reviewsandratings"
...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields('updatedDTS').filter(r.row('updatedDTS').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).map(function(row){return row.merge({'updatedDTS' : row('updatedDTS').toISO8601()})})" > $fileName
