                                                                                                                ...
                                                                                                                PREVIOUS_DATE=`date --date "$TIME_DIFFERENCE $TIME_TYPE ago" +%Y,%-m,%-d,%-H,%-M,%-S`
                                                                                                                        echo "Export for data started"
                                                                                                                                echo "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('lastUpdated').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z')))"
                                                                                                                                        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').map(function(row){return row.merge({'lastUpdated' : row('lastUpdated').toISO8601().slice(0,-6),'createdDTS' : row('createdDTS').toISO8601().slice(0,-6)})})" > $fileName
