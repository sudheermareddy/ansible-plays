...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').map(function(row){return row.merge({'updated' : row('updated').toISO8601().slice(0,-6)})})" > $fileName
