...
CONTAINER_NAME="tl-reports/skill-tag"

HOST="rdbokenprtl"
#HOST="rdbprtl"
PORT="28015"
DB_NAME="tl_veegam_metadata"
TABLE_NAME="skills"
...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('updated').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).map(function(row){return row.merge({'updated' : row('updated').toISO8601().slice(0,-6)})})" > $fileName
