...
TABLE_NAME="channels"
CONTAINER_NAME="tl-reports/channel" # container name changes
...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields('createdDTS').map(function(row){return row.merge({'createdDTS' : row('createdDTS').date().toISO8601().slice(0,-6)})})" > $fileName
