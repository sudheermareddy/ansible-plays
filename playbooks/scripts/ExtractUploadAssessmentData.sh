...
CONTAINER_NAME="tl-reports/assessment"
												        ...
												        CURRENT_DATE=`date +%Y,%-m,%-d,%-H,%-M,%-S`
													        PREVIOUS_DATE=`date --date "$TIME_DIFFERENCE $TIME_TYPE ago" +%Y,%-m,%-d,%-H,%-M,%-S`
														        echo "Export for data started"
															        echo "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('updated').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z')))"
																        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields({'published': {'name': true}}).filter(r.row('updated').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).map(function(row){return row.merge({'updated' : row('updated').toISO8601().slice(0,-6), 'created' : row('created').toISO8601().slice(0,-6)})})" > $fileName
