													        ...
													        PREVIOUS_DATE=`date --date "$TIME_DIFFERENCE $TIME_TYPE ago" +%Y,%-m,%-d,%-H,%-M,%-S`
														        echo "Export for data started"
															        echo "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('updatedDTS').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z')))"
																        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('updatedDTS').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).map(function(row){return row.merge({'createdAt' : row('createdAt').toISO8601().slice(0,-6),'updatedDTS' : row('updatedDTS').toISO8601().slice(0,-6)})})" > $fileName
