													        ...
													        PREVIOUS_DATE=`date --date "$TIME_DIFFERENCE $TIME_TYPE ago" +%Y,%-m,%-d,%-H,%-M,%-S`
														        echo "Export for data started"
															        echo "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('launchTime').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z')))"
																        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').hasFields('trainingRequestedTimestamp','launchTime','cleanupTime','trainingInitRequestedTimestamp').filter(r.row('launchTime').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).pluck('launchState','trainingRequestedTimestamp','user_training_id','orgId','failureReason','duration','userEmail','userId','userName','modeType','finalResponse','geoip','transfer_group','trainingDeploymentId','launchTime','cleanupStatus','channelData','publisherOrg','isEmailSent','labId','labName','cleanupTime','labType','paymentStatus','providerCleanup','traceId','trainingInitRequestedTimestamp','version','geoip').merge(function (doc){ return{ 'city' : doc('geoip')('city_name'), 'country' : doc('geoip')('country_name')}}).map(function(row){return row.merge({'trainingRequestedTimestamp' : row('trainingRequestedTimestamp').default(r.time(9999, 12, 31, 'Z')).toISO8601().slice(0,-6),'launchTime' : row('launchTime').default(r.time(9999, 12, 31, 'Z')).toISO8601().slice(0,-6),'cleanupTime' : row('cleanupTime').default(r.time(9999, 12, 31, 'Z')).toISO8601().slice(0,-6),'trainingInitRequestedTimestamp' : row('trainingInitRequestedTimestamp').default(r.time(9999, 12, 31, 'Z')).toISO8601().slice(0,-6)})})" > $fileName
