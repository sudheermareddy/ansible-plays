...
then
        ...
        /usr/local/bin/recli -h $HOST -p $PORT --json "r.db('$DB_NAME').table('$TABLE_NAME').filter(r.row('updatedDTS').during(r.time($PREVIOUS_DATE,'Z'), r.time($CURRENT_DATE,'Z'))).hasFields({'status': {'publish' : { 'createdDTS': true}}}).pluck({'createdDTS': true,'updatedDTS': true,'status': {'publish' : { 'labId': true, 'skills': true, 'authors': true, 'category': true, 'createdDTS': true, 'labName': true, 'labType': true, 'longDescription': true, 'orgId': true, 'publishType': true, 'publisherName': true, 'shortDescription': true, 'skillLevel': true, 'status': true, 'tags': true, 'tenantId': true, 'userId': true, 'updatedDTS': true, 'updatedBy': true, 'createdBy': true} }}).map(function(data){ return data('status')('publish').merge({'createdDTS':data('createdDTS').date().toISO8601(),'updatedDTS' : data('updatedDTS').date().toISO8601() })})" > $fileName
